...
class UIFunctions(MainWindow):
    ...
    def addNewMenu(self, name, objName, icon, isTopMenu):
        ...
        if isTopMenu:
            self.ui.layout_menus.addWidget(button,1)
        else:
    ...
    def selectMenu(getStyle):
        select = getStyle + ("QPushButton { border-right: 5px solid rgb(20, 255, 236); }")
        return select

    ## ==> DESELECT
    def deselectMenu(getStyle):
        deselect = getStyle.replace("QPushButton { border-right: 5px solid rgb(20, 255, 236); }", "")
        return deselect
